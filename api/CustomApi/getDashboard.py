...
class GetDashboard(APIView):
    ...
    def get(self, request):
        ...
        # ── Executions (from Workflow counters) ─────────────────────────────
        wf_exec_qs = Workflow.objects.all() if is_super else Workflow.objects.filter(created_by=user)
        exec_agg = wf_exec_qs.aggregate(
            total_executions=Sum('execution_count'),
            total_actions_consumed=Sum('total_actions_consumed'),
        ...
        executions = {
            'total_executions':       exec_agg['total_executions'] or 0,
            'total_actions_consumed': exec_agg['total_actions_consumed'] or 0,
            'by_status': {
                'completed': exec_agg['completed'],
